...
public class frmMunicipalidadVillaRegina : Form
{
	...
	public frmMunicipalidadVillaRegina(eUsuario pUsuarioLogueado)
	{
		InitializeComponent();
	...
	private void frmMunicipalidadVillaRegina_Load(object sender, EventArgs e)
	{
		txtNuevoCodigo.KeyDown += txt_KeyDown;
		...
		listaDisponibles = new BindingList<eLote>(nLotes.obtenerLotesDisponibleSeparacion(oUsuarioLogueado, 0));
	...
	private void actualizarLotesDisponibles()
	{
		...
		List<eLote> listaDesdeBD = nLotes.obtenerLotesDisponibleSeparacion(oUsuarioLogueado, 0);
		...
		try
		{
			...
			foreach (DataGridViewRow row in (IEnumerable)dgvIndicesEncontrados.Rows)
			{
				...
				tareas.Add(Task.Run(async delegate
				{
					try
					{
						List<eCodigosBarrasEncontrados_v3> resultado = await nCodigoBarras_Muni_v2.buscarCodigoBarrasAsync(pNombreLote: row.Cells[0].Value.ToString(), pNombreArchivo: row.Cells[1].Value.ToString(), pRutaArchivoPDF: row.Cells[2].Value.ToString(), pUsuarioLogueado: oUsuarioLogueado);
	...
	private void PdfViewer_MouseClick(object sender, MouseEventArgs e)
	{
		...
		float zoomFactor;
		if (txtSeleccion.Text == "ZOOM" && sender is PictureBox pictureBox)
		{
			...
			using (Image bitmap = pdfDocument.Render(pageIndex, 4f, 4f, forPrinting: true))
			{
				zoomImage = new Bitmap(bitmap);
			}
		...
		{
			zoomFactor *= factor;
			zoomPictureBox.Width = (int)((float)zoomImage.Width * zoomFactor);
			zoomPictureBox.Height = (int)((float)zoomImage.Height * zoomFactor);
		}
	}
	...
	private void procesarTodo()
	{
		...
		nLotes.separarArchivosLote(oUsuarioLogueado, oListaLotesSeleccionadosProcesar, listaSeperadores);
	}
	...
	private void frmMunicipalidadVillaRegina_FormClosing(object sender, FormClosingEventArgs e)
	{
		isDisposing = true;
	...
	{
		System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(Capturador._01_Pantallas._04_Separador.frmMunicipalidadVillaRegina));
		...
		this.btnAsignarCodigo.TabIndex = 2;
		...
		this.btnCancelarAsignar.TabIndex = 3;
		...
		base.Name = "frmMunicipalidadVillaRegina";
		base.StartPosition = System.Windows.Forms.FormStartPosition.CenterScreen;
		this.Text = "Procesar Municipalidad de Villa Regina";
		base.FormClosing += new System.Windows.Forms.FormClosingEventHandler(frmMunicipalidadVillaRegina_FormClosing);
		base.Load += new System.EventHandler(frmMunicipalidadVillaRegina_Load);
