...
public class frmOCR_v2 : Form
{
	...
	public frmOCR_v2(eUsuario pUsuarioLogueado)
	{
		InitializeComponent();
	...
	private void frmOCR_v2_Load(object sender, EventArgs e)
	{
	...
	{
		bool tesseractOk = VerificarComando("tesseract");
		bool ocrmypdfOk = VerificarComando("ocrmypdf");
		if (!tesseractOk || !ocrmypdfOk)
		{
			MessageBox.Show("Faltan dependencias: asegúrese de tener instalado Tesseract y OCRmyPDF", "Dependencias faltantes", MessageBoxButtons.OK, MessageBoxIcon.Hand);
			Close();
		}
	}

	private bool VerificarComando(string comando)
	{
		try
		{
	...
	private async void btnProcesar_Click_1(object sender, EventArgs e)
	{
		...
		{
			oProyectoConfiguracion = nConfiguracion.ObtenerUltimaCarpeta(oUsuarioLogueado, Convert.ToInt32(row.Cells["cdProyecto"].Value));
			string rutaLoteContar = ((!(oProyectoConfiguracion.snSeparacion == "SI")) ? Convert.ToString(row.Cells["dsRutaLote"].Value) : Convert.ToString(row.Cells["dsRutaLoteFinal"].Value));
		...
		foreach (DataGridViewRow row2 in (IEnumerable)dgvLotesSeleccionados.Rows)
		{
			...
			if (oProyectoConfiguracion.snSeparacion == "SI")
			{
				oLoteSeleccionado.dsRutaLote = Convert.ToString(row2.Cells["dsRutaLoteFinal"].Value);
			}
			else
			{
				oLoteSeleccionado.dsRutaLote = Convert.ToString(row2.Cells["dsRutaLote"].Value);
			}
			oLoteSeleccionado.dsProyecto = Convert.ToString(row2.Cells["dsProyecto"].Value);
			oLoteSeleccionado.dsNombreLote = Convert.ToString(row2.Cells["dsNombreLote"].Value);
			oLoteSeleccionado.nuCantidadArchivos = Convert.ToInt32(row2.Cells["nuCantidadArchivos"].Value);
			oLoteSeleccionado.dsRutaLoteFinal = Path.Combine(txtCarpetaSalida.Text, Convert.ToString(row2.Cells["dsNombreLote"].Value));
			try
			{
	...
	private async Task procesarLotes(eLote pLote)
	{
		...
		if (Directory.Exists(rutaLoteDestino))
		{
			throw new Exception("La carpeta de destino ya existe. Seleccione otra carpeta");
		}
		...
		{
			throw new Exception("No se encontraron archivos PDF en la carpeta " + pLote.dsRutaLote);
		}
		await Task.Run(delegate
		{
			...
			foreach (string text in array)
			{
				...
				try
				{
					AplicarOCRyGuardarPDF(text, rutaPDFDestino);
				}
				catch
				{
				}
	...
	{
		if (!EjecutarOCRMyPDF(rutaPDFOrigen, rutaPDFDestino, out var salidaOCR))
		{
			Console.WriteLine("OCR falló para: " + rutaPDFOrigen);
			Console.WriteLine(salidaOCR);
		}
	}

	private bool EjecutarOCRMyPDF(string rutaOriginal, string rutaConvertido, out string salidaConsola)
	{
		salidaConsola = "";
		try
		{
			ProcessStartInfo processStartInfo = new ProcessStartInfo();
			processStartInfo.FileName = "C:\\Program Files\\Python313\\Scripts\\ocrmypdf.exe";
			processStartInfo.Arguments = "--language spa \"" + rutaOriginal + "\" \"" + rutaConvertido + "\" --skip-text";
			...
			string salidaStd = process.StandardOutput.ReadToEnd();
			string salidaErr = process.StandardError.ReadToEnd();
			process.WaitForExit();
			salidaConsola = salidaStd + "\n" + salidaErr;
			return process.ExitCode == 0;
		}
		catch (Exception ex)
		{
			throw new Exception("Error al ejecutar OCRMyPDF: " + ex.Message);
		}
	}
	...
	{
		System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(Capturador._01_Pantallas._08_OCR.frmOCR_v2));
		...
		this.Text = "Convertir Lotes a PDF con OCR (v2)";
		base.Load += new System.EventHandler(frmOCR_v2_Load);
